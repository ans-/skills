...
| `podcast-lang-mark` | Annotates podcast shows in an xlsx export with `lang_target`/`lang_from` ISO language codes, a confidence score, and a self-updating Excel SQL-update formula column, judging each show from its title, author, and summary, then reporting low-confidence rows for human review. |
